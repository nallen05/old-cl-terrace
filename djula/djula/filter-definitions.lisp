...
(def-filter :force_escape (it)
  (djula-util:html-escape (princ-to-string it)))
...
note: lisp filters can be turned off by setting the variable *TEMPLATE-EVAL* to NIL
")

(def-filter :lisp (it &optional lisp-string)
  (if (not *template-eval*)
      (with-template-error #1=(template-error-string "I can't evaulate the \"lisp\" filter ~A because *TEMPLATE-EVAL* is NIL"
						     lisp-string)
			   (error #1#))
      (with-template-error (template-error-string "There was an error reading the lisp string ~S"
						  lisp-string)
	(let* ((*package* (find-package :cl-user))
	       (sexp (read-from-string lisp-string)))
	  (with-template-error (template-error-string "There was an error interpreting the lisp expression ~S"
						      sexp)
	    (let ((fn (coerce sexp 'function)))
	      (with-template-error (template-error-string "There was an error executing the lisp expression ~S"
							  sexp)
		(funcall fn it))))))))
...
(def-filter :urlencode (it)
  (djula-util:url-encode (princ-to-string it)))