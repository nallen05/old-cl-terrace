...
(defun .get-special-binding-alist (test-folder)
  (let ((p (cl-fad:file-exists-p (merge-pathnames "bindings.sexp" test-folder))))
    (if p
	(eval (read-from-string (format nil "(list ~A)" (djula-util:slurp-utf-8-file p)))))))
...
(defun .render-test-template (named test-folder &key return-bytes)
  (multiple-value-bind (vars vals)
    ...
    (progv vars vals
      (apply (djula:compile-template (merge-pathnames named test-folder)
				     :template-folder test-folder
				     :format return-bytes)
	     (let ((args.sexp (cl-fad:file-exists-p (merge-pathnames "args.sexp" test-folder))))
	       (if args.sexp
		   (read-from-string (djula-util:slurp-utf-8-file args.sexp))))))))

(defun .run-compile-template-test/string (test-folder)
  (let ((out.txt (djula-util:slurp-utf-8-file (merge-pathnames "out.txt" test-folder))))
...
(defun .run-compile-template-test/bytes (test-folder)
  (let ((out.txt/bytes (trivial-utf-8:string-to-utf-8-bytes (djula-util:slurp-utf-8-file (merge-pathnames "out.txt" test-folder)))))
